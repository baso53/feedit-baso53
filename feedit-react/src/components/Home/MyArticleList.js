...
class MyArticleList extends Component {
    ...
    getArticlesVanilla(changePage = 0) {
        ...
        axios.get('/api/articles/search/findByUsernameid', {
            params: {
    ...
    changeSelected (articleId){
        this.props.changeSelected(articleId);
    }
    ...
    handleSearch(event, data) {
        if (data.value.length > 2) {
            axios.get('/api/articles/search/findByTitleIgnoreCaseContainingAndUsernameid', {
                params: {
                    title: data.value,
                    usernameid: this.props.userId,
                    size: this.state.page.size,
